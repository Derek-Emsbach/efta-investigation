...
threads:
  - id: "THREAD_01"
    name: "Staley — Trustee, Banker, Accused Rapist"
    file: "THREAD_01_Staley_Trustee.md"
  - id: "THREAD_02"
    name: "The Dubin Architecture"
    file: "THREAD_02_Dubin_Architecture.md"
  - id: "THREAD_03"
    name: "Witness Control Mechanisms"
    file: "THREAD_03_Witness_Control.md"
  - id: "THREAD_04"
    name: "Indyke Conflicts of Interest"
...
This brief assigns evidence-based tiers to every named entity. Tiers reflect **strength of documented evidence**, not guilt.
...
2. **The trust's primary beneficiary family (the Dubins) included the residuary heir (Celina, $250M+), the successor trustee (Eva), and a member with Tier 1 criminal exposure (Glenn) — creating a structural alignment where the family with the most to inherit also had the most to lose from scrutiny.** (THREAD_02)

3. **The trust embedded a legally binding witness control system — the employment cliff, the golden handcuffs, and the no-contest clause — that financially punished cooperation with authorities and rewarded silence. These structural provisions operated alongside active obstruction by the same individuals who controlled bequests.** (THREAD_03)

4. **A co-trustee (Staley) was named in the prosecution memo as having raped a victim during a directed massage, corroborated by JPMorgan communications. He signed the trust three times (including the amendment adding witness control provisions) and no resignation has been found in 1.38 million documents.** (THREAD_01)

5. **The prosecution memo documented 38 victims, corroborated sexual assault by multiple named individuals, and evidence of obstruction — then charged only one person. A separate three-year investigation accumulated forensically authenticated victim journals and multiple corroborating accounts — and the case handler never wrote a formal assessment.** (THREAD_05)
...
Three threads converge on Darren Indyke as the central node of the system:
...
The prosecution memo's charging analysis (pages 68-85) is the most consequential redacted section. It hides:
...
Drawn from all five threads, prioritized by analytical impact:

| # | Thread | Priority | Question |
|---|--------|----------|----------|
| 1 | T05 | P1 | What do the redacted pages 74-85 of the prosecution memo say about the three redacted subjects and Leslie Groff? |
| 2 | T01 | P1 | Did Staley ever resign as trustee? If not, did he serve while CEO of Barclays? |
...
| 10 | T03 | P2 | Who are the A.36 and A.37 beneficiaries (the two-year golden handcuffs)? |
...
## Appendix A: Source Document Register

| EFTA Number | Title | Date | Dataset | Pages | Threads |
|-------------|-------|------|---------|-------|---------|
| EFTA01266380 | The Jeffrey E. Epstein 2014 Trust | Nov 18, 2014 | DS10 | 23 | 01, 02, 03, 04 |
| EFTA01266403 | A&R of the Epstein 2014 Trust | May 1, 2015 | DS10 | 24 | 01, 02, 03, 04 |
| EFTA01266427 | First Amendment to the A&R | ~Sep 2015 | DS10 | 6 | 01, 03, 04 |
| EFTA02731082 | SDNY Prosecution Memo | Dec 19, 2019 | DS12 | 86 | 01, 02, 03, 04, 05 |
| EFTA00156644 | DANY/FBI Briefing | Jun 2023 | DS9 | 10 | 01, 05 |
| EFTA00162121 | USVI v. JPMorgan Complaint | Jan 2023 | DS9 | 253 | 01 |
| EFTA01582849 | JPMorgan Compliance DDR | Feb 2009 | DS10 | 1 | 01 |

## Appendix B: Entity Quick Reference

| Name | Tier | Label | UUID | Threads |
|------|------|-------|------|---------|
| Jeffrey Epstein | 1 | DIRECT | a1b2c3d4-... | 01, 02, 03, 04, 05 |
| Jes Staley | 1 | DIRECT | 32632113-... | 01, 03, 05 |
| Leon Black | 1 | DIRECT | — | 05 |
| Glenn Dubin | 1 | DIRECT | c8b4057b-... | 02, 05 |
| Ghislaine Maxwell | 1 | DIRECT | — | 01, 05 |
| Jean-Luc Brunel | 1 | DIRECT | — | 04 |
| Harvey Weinstein | 1 | DIRECT | — | 05 |
| Prince Andrew | 3 | CIRCUMSTANTIAL | — | 05 |
| Lesley Groff | 2 | IMMUNIZED | b2c3d4e5-... | 01, 03, 05 |
| Darren Indyke | 6 | PERIPHERAL | ff53f22d-... | 01, 03, 04, 05 |
| Richard D. Kahn | 6 | PERIPHERAL | c9d3f9d3-... | 03, 04 |
| David Mitchell | 6 | PERIPHERAL | — | 01, 04 |
| Eva Andersson-Dubin | 4 | ASSOCIATED | — | 02, 03 |
| Celina Edith Dubin | 4 | ASSOCIATED | — | 02 |
| Karyna Shuliak | 4 | ASSOCIATED | — | 04 |

## Appendix C: Methodology + Corpus Search Reproducibility

### Tools Used

| Tool | Purpose |
|------|---------|
| SQLite corpus (6.3GB) | Full-text search across 1.38M documents via FTS5 |
| Supabase MCP server (34 tools) | Entity lookup, document retrieval, evidence linking |
| R2 text extraction | Full text access for documents beyond 2K DB preview |
| Three-way document comparison | Manual side-by-side analysis of trust versions |

### Key Corpus Queries

These queries can be reproduced using the `corpus_search` and `corpus_get_document_text` tools in the EFTA MCP server:

| Query | Purpose | Result |
|-------|---------|--------|
| `corpus_search("Staley")` | Locate all Staley references | 7,058 documents, 7,317 pages |
| `corpus_search("trustee resign", efta_number="EFTA01266...")` | Search for trustee resignation | 0 results |
| `corpus_get_document_text("EFTA01266380", 0, 0)` | Confirm Staley in original trust | Page 0: "DARREN K. INDYKE, JAMES E. STALEY and DAVID MITCHELL, as Trustees" |
| `corpus_get_document_text("EFTA02731082", 32, 32)` | Read Staley rape account | Full victim testimony |
| `corpus_get_document_text("EFTA01266427", 1, 2)` | Read employment cliff text | Section 2.5 full provision |
| `corpus_search("Leon Black", dataset=12)` | Locate Black prosecution documents | DS12 communications |
| `corpus_search("Celina Dubin")` | Confirm beneficiary across documents | Trust documents + minimal other results |
| `corpus_search("Barbro")` | Identify "Barbro and the BBB's" | Open — not yet fully resolved |

### Reproducibility Notes

- All EFTA numbers are direct references to DOJ-hosted PDFs at `https://www.justice.gov/epstein/files/DataSet%20{N}/{EFTA_NUMBER}.pdf`
- Corpus page numbers are 0-indexed (page 0 = first page of document)
- OCR quality varies — trust documents have ~5% garbled characters; prosecution memo is substantially clean
- Redaction analysis is based on visible content only; OCR artifacts near redaction bars may produce false entities
