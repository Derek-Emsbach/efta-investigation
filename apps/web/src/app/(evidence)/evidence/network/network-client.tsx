...
export default function EvidenceNetworkClient() {
  ...
  useEffect(() => {
    ...
    const label = g.append('g')
      ...
      .attr('opacity', (d) => pathResult ? (pathResult.nodeIds.has(d.id) ? 1 : 0.1) : 1)
      .attr('text-anchor', 'middle')
    ...
    node
      .on('mouseover', (_event, d) => {
        ...
        node.attr('opacity', (n) => connIds.has(n.id) ? 1 : 0.2)
        label.attr('opacity', (n) => connIds.has(n.id) ? 1 : 0.15)
      })
      .on('mouseout', () => {
        setHoveredNode(null)
        link
      ...
      .on('click', (_event, d) => {
        // Navigate to evidence room entity profile
        if (d.slug) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      ...
      <div ref={containerRef} className="bg-surface border border-border-default rounded-lg overflow-hidden relative min-h-[600px]">
        {filteredData && filteredData.nodes.length > 0 ? (
          <>
            ...
            {/* Hover tooltip */}
            {hoveredNode && (
              <div className="absolute top-4 right-4 bg-elevated border border-border-default rounded-lg p-3 min-w-[180px] pointer-events-none shadow-lg">
                ...
                {hoveredNode.slug && <p className="text-xs text-critical mt-1.5 font-mono">Click to view profile</p>}
              </div>
            )}
          </>
        ) : (
      ...
      <div className="flex flex-wrap items-center gap-4 mt-4 text-xs text-text-muted font-mono">
        ...
        <span className="text-text-secondary ml-4">Strength:</span>
